...
public class TestJSONObject extends TestCase {
	...
	public void testMultipleThings() {
		try {
			jsonobject = new JSONObject("{foo: [true, false,9876543210,    0.0, 1.00000001,  1.000000000001, 1.00000000000000001,"
	...
	public void testWrite() {
		try {
			...
			jsonarray = jsonobject.getJSONArray("stooge");
			jsonarray.put(5, "Shemp");
			assertEquals("{\"stooge\":[\"Curly\",\"Larry\",\"Moe\",null,null,\"Shemp\"]}", jsonobject.write(new StringWriter()).toString());
		} catch (JSONException e) {
			fail(e.getMessage());
		}
	}
	...
	public static void testConstructor_FrenchResourceBundle() {
		try {
			...
			assertEquals(
					...
					new JSONObject("org.json.tests.SampleResourceBundle", currentLocale).toString());
		} catch (JSONException e) {
			fail(e.getMessage());
		}
	}
	...
	public void testConstructor_ObjectWithStringArray() {
		assertEquals("{\"m\":true,\"i\":3}", new JSONObject(new ObjectWithPrimatives(), new String[] { "i", "m", "k" }).toString());
	}
	...
	public void testWrap() {
		try {
			...
			assertEquals("javax.print.PrintException", JSONObject.wrap(new javax.print.PrintException()));
	...
	public void testAppend() {
		try {
			...
			assertEquals("{\"george\":[\"def\"],\"string\":\"123\",\"jsonarray\":[\"abc\",\"123\"]}", jsonobject.toString());
		} catch (JSONException e) {
			fail(e.getMessage());
		}
	}
	...
	public void testConstuctor_Strings() {
		...
		try {
			...
			assertEquals("Expected a ',' or '}' at 10 [character 11 line 1]", e.getMessage());
		}
		try {
			jsonobject = new JSONObject("{\"123\",34}");
			fail("Should have thrown exception.");
		} catch (JSONException e) {
			assertEquals("Expected a ':' after a key at 7 [character 8 line 1]", e.getMessage());
		}
		try {
			jsonobject = new JSONObject("{");
			fail("Should have thrown exception.");
		} catch (JSONException e) {
			assertEquals("A JSONObject text must end with '}' at 2 [character 3 line 1]", e.getMessage());
		}
	}
	...
	public void testConstructor_CopySubset() {
		try {
			...
			assertEquals("{\"abc\":\"123\",\"abcde\":\"12345\"}", new JSONObject(jsonobject, new String[] { "abc", "abc", "abcde" }).toString());
		} catch (JSONException e) {
			fail(e.getMessage());
		}
	}
	...
	public void testToJsonArray() {
		try {
			...
			jsonobject.put("abcde", "12345");
			jsonobject.put("abcdef", "123456");
			assertEquals("[\"123\",\"123\",\"12345\"]", jsonobject.toJSONArray(new JSONArray(new String[] { "abc", "abc", "abcde" })).toString());
	...
	public void testValueToString_Object() {
		try {
			...
			jsonobject = new JSONObject();
			jsonobject.put("abc", "123");
			assertEquals("{\"abc\":\"123\"}", JSONObject.valueToString(jsonobject));
			...
			try {
				...
				assertEquals("Bad value from toJSONString: null", e.getMessage());
			}

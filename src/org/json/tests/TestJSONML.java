...
public class TestJSONML extends TestCase {
	...
	public void testToJsonArray_SpanishNumbers() {
		try {
			...
			assertEquals(
					"[\n    \"xml\",\n    {\"do\": 0},\n    \"uno\",\n    [\n        \"a\",\n        {\n            \"re\": 1,\n            \"mi\": 2\n        },\n        \"dos\",\n        [\n            \"b\",\n            {\"fa\": 3}\n        ],\n        \"tres\",\n        [\n            \"c\",\n            true\n        ],\n        \"quatro\"\n    ],\n    \"cinqo\",\n    [\n        \"d\",\n        \"seis\",\n        [\"e\"]\n    ]\n]",
					jsonarray.toString(4));
			assertEquals("<xml do=\"0\">uno<a re=\"1\" mi=\"2\">dos<b fa=\"3\"/>tres<c></c>quatro</a>cinqo<d>seis<e/></d></xml>", JSONML.toString(jsonarray));
		} catch (JSONException e) {
			fail(e.getMessage());
		}
	}
	...
	public void testToJsonObject_MessageXml() {
		try {
			...
			assertEquals(
					...
					jsonobject.toString());
			jsonarray = JSONML.toJSONArray(string);
			assertEquals("[\"Root\",[\"MsgType\",{\"type\":\"node\"},[\"BatchType\",{\"type\":\"string\"},111111111111111]]]", jsonarray.toString());
		} catch (JSONException e) {
			fail(e.getMessage());
		}
	}
	...
	public void testToJSONArray_MetaTagWithTwoDashes() {
		...
		try {
			...
			assertEquals("<abc>&gt;</abc>", JSONML.toString(jsonarray));
		} catch (JSONException e) {
			fail(e.getMessage());
		}
	}
	...
	public void testToJSONArray_MetaTagWithOneDash() {
		...
		try {
			jsonarray = JSONML.toJSONArray("<abc><!-abc--></abc>");
			assertEquals("[\"abc\",\"abc-->\"]", jsonarray.toString());
			assertEquals("<abc>abc--&gt;</abc>", JSONML.toString(jsonarray));
		} catch (JSONException e) {
			fail(e.getMessage());
		}
	}
	...
	public void testToJSONArray_MetaTagWithCdata() {
		...
		try {
			...
			assertEquals("[\"abc\",\"<abc><\\/abc>\"]", jsonarray.toString());
			assertEquals("<abc>&lt;abc&gt;&lt;/abc&gt;</abc>", JSONML.toString(jsonarray));
		} catch (JSONException e) {
			fail(e.getMessage());
		}
	}
	...
	public void testToJSONArray_MetaTagWithCdataOnly() {
		...
		try {
			...
			assertEquals("[\"abc\"]", jsonarray.toString());
			assertEquals("<abc/>", JSONML.toString(jsonarray));
		} catch (JSONException e) {
			fail(e.getMessage());
		}
	}
	...
	public void testToJSONObject_NoValueAttribute() {
		try {
			...
			assertEquals("{\"novalue\":\"\",\"tagName\":\"abc\",\"childNodes\":[\"def\"]}", jsonobject.toString());
			assertEquals("<abc novalue=\"\">def</abc>", JSONML.toString(jsonobject));
		} catch (JSONException e) {
			fail(e.getMessage());
		}
	}
	...
	public void testToJSONObject_EmptyTag() {
		try {
			...
			assertEquals("{\"tagName\":\"abc\"}", jsonobject.toString());
			assertEquals("<abc/>", JSONML.toString(jsonobject));
		} catch (JSONException e) {
			fail(e.getMessage());
		}
	}
	...
	public void testToString_JsonArrayOfJsonObjects() {
		try {
			...
			assertEquals("<tagName tagName=\"myName\"><otherName/></tagName>", JSONML.toString(jsonarray));
		} catch (JSONException e) {
			fail(e.getMessage());
		}
	}
	...
	public void testToString_JsonObjectOfJsonArrays() {
		try {
			...
			assertEquals("<MyName 123=\"[&quot;abc&quot;]\">abc<def/></MyName>", JSONML.toString(jsonobject));
		} catch (JSONException e) {
			fail(e.getMessage());
		}
	}
